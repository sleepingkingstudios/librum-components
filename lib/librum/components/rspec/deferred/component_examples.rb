  ...
  module ComponentExamples
    ...
    do |allow_extra_options: false, has_required_keywords: false, layout: false|
      ...
      describe '#safe_buffer' do
        it 'should define the method' do
          expect(component)
            .to respond_to(:safe_buffer)
            .with(0).arguments
            .and_a_block
        end

        it { expect(component.safe_buffer).to be_a ActiveSupport::SafeBuffer }

        it { expect(component.safe_buffer.size).to be 0 }

        describe 'with a block' do
          it 'should yield the buffer' do
            expect { |block| component.safe_buffer(&block) }
              .to yield_with_args(an_instance_of(ActiveSupport::SafeBuffer))
          end

          it 'should return a safe buffer' do
            expect(component.safe_buffer(&nil))
              .to be_a ActiveSupport::SafeBuffer
          end

          it { expect(component.safe_buffer(&nil).size).to be 0 }
          ...
          context 'when the block modifies the buffer' do
            ...
            it 'should return the buffer' do
              expect(component.safe_buffer(&block))
                .to be_a ActiveSupport::SafeBuffer
            end

            it 'should yield and return the buffer' do
              expect(component.safe_buffer(&block).to_s).to be == expected
            end
          end
        end
      end
      ...
      describe '#safe_buffer?' do
        it { expect(component).to respond_to(:safe_buffer?).with(1).argument }

        it { expect(component.safe_buffer?(nil)).to be false }

        it { expect(component.safe_buffer?(Object.new.freeze)).to be false }

        it { expect(component.safe_buffer?(String)).to be false }

        it { expect(component.safe_buffer?(component.safe_buffer)).to be true }
      end
      ...
      describe '#strip_buffer' do
        it { expect(component).to respond_to(:strip_buffer).with(1).argument }

        define_method :buffer do |value|
          component.safe_buffer << value
        end
        ...
        describe 'with nil' do
          ...
          it 'should raise an exception' do
            expect { component.strip_buffer(nil) }
              .to raise_error ArgumentError, error_message
          end
        end
        ...
        describe 'with an Object' do
          ...
          it 'should raise an exception' do
            expect { component.strip_buffer(Object.new.freeze) }
              .to raise_error ArgumentError, error_message
          end
        end
        ...
        describe 'with an empty String' do
          ...
          it { expect(component.strip_buffer(value)).to be == value }
        end
        ...
        describe 'with a String with leading whitespace' do
          ...
          it { expect(component.strip_buffer(value)).to be == expected }
        end
